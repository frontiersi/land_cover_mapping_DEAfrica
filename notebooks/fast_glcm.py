...
def fast_glcm_contrast(img, vmin=0, vmax=255, levels=8, ks=5, distance=1.0, angles=[0.0]):
    '''
    calc glcm contrast
    '''
    h,w = img.shape
    cont = np.zeros((h,w,len(angles)), dtype=np.float32)
    for k in range(len(angles)):
        glcm = fast_glcm(img, vmin, vmax, levels, ks, distance, angles[k])
        for i in range(levels):
            for j in range(levels):
                cont[:,:,k] += glcm[i,j] * (i-j)**2

    return np.mean(cont,axis=2)

def fast_glcm_correlation(img, vmin=0, vmax=255, levels=8, ks=5, distance=1.0, angles=[0.0]): # added, needs to review
    '''
    calc glcm correlation
    '''
    h,w = img.shape
    corr = np.zeros((h,w,len(angles)), dtype=np.float32)
    for k in range(len(angles)):
        glcm = fast_glcm(img, vmin, vmax, levels, ks, distance, angles[k])
        ...
        for i in range(levels):
            for j in range(levels):
                corr[:,:,k] += glcm[i,j] * (i-mean)*(i-mean)/(std2+0.00001) # avoid dividing by zero

    return np.mean(corr,axis=2)
